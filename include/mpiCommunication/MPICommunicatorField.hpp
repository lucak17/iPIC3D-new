...
struct typeFaces : std::array<std::array<int,3>,6> {
    inline constexpr typeFaces() 
...
struct typeEdges : std::array<std::array<int,3>,12> {
    inline constexpr typeEdges() 
    : std::array<std::array<int,3>,12> {{ 
...
struct typeCorners : std::array<std::array<int,3>,8> {
    inline constexpr typeCorners() 
...
public:
    MPICommunicatorField(uint nx, uint ny, uint nz, uint nw, uint haloX, uint haloY, uint haloZ, uint haloW, const VirtualBuffer<T,Dim,unified>& buffer)
    {
        halo_[0] = haloX;
        halo_[1] = haloY;
        halo_[2] = haloZ;
                ...
                for(uint diry = 0; diry<communicationDim; diry++ ){
                    for(uint dirx = 0; dirx<communicationDim; dirx++ ){
                        ...
                        for(uint d = 0; d<communicationDim; d++){
                            idxStartSend[d] = dir[d] > 0 ? extentNoHalo[d] - halo_[d] : 0;
                            // last data to send has idx = idxEndSend - 1
                            idxEndSend[d] = dir[d] == 0 ? extentNoHalo[d] : idxStartSend[d]+halo_[d];
                            // idxStartRcv[d] = dir[d] < 0 ? idxStartSend[d] - halo_[d] : ( dir[d] == 0 ? 0 : idxEndSend[d]);
                        ...
                        for(uint k = idxStartSend[2]; k<idxEndSend[2]; k++){
                            for(uint j = idxStartSend[1]; j<idxEndSend[1]; j++){
                                for(uint i = idxStartSend[0]; i<idxEndSend[0]; i++){
                                    uint iShift = i + halo_[0];
                                    uint jShift = j + halo_[1];
                                    uint kShift = k + halo_[2];
                                    border_indx[idxLoop] = buffer.get1DFlatIndex(iShift, jShift, kShift);
                                    halo_indx[idxLoop] = buffer.get1DFlatIndex(iShift + dir[0]*halo_[0], jShift + dir[1]*halo_[1], kShift + dir[2]*halo_[2]);
                                    idxLoop++;
                                }
                            }
                        }
    ...
    void computeNeighbourRank(){
        ...
        constexpr typeFaces faces;
        constexpr typeEdges edges;
        constexpr typeCorners corners;
        for (auto const& [dirx,diry,dirz] : faces) computeNghb(dirx,diry,dirz);
        for (auto const& [dirx,diry,dirz] : edges) computeNghb(dirx,diry,dirz);
        for (auto const& [dirx,diry,dirz] : corners) computeNghb(dirx,diry,dirz);
    }
    ...
    int copyBorderToHaloSelf(T* data){
        ...
        auto copyBorderHalo = [&](int dirx,int diry,int dirz) -> void {
            const int idxDirectionComm = (dirx + 1) + (diry + 1) * communicationDim + (dirz + 1) * communicationDim * communicationDim;
            assert(idxDirectionComm != 13);
            if constexpr(onlyCommunicationSides){
                if(neighbour_[idxDirectionComm] < 0)return;
            }
